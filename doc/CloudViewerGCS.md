...
> **teslausb-ng note:** the web UI exposes a single **Local / Cloud** switch
> (top of the Viewer tab). "Cloud" uses the provider set by `CLOUD_PROVIDER`
> (the default provider). The API still supports addressing a specific provider
> via the `?provider=gcs|s3` query parameter if you script against it directly.
> The switch only appears when served by this Docker stack with a configured
> cloud source — the Pi's own web UI is unaffected.
